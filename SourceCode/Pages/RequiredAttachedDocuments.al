page 60000 "Required Attached Documents"
{
    Caption = 'Required Attached Documents';
    DelayedInsert = true;
    Editable = true;
    PageType = ListPart;
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies file type of the attachment.';
                }
                field(Description; Rec.Description)
                {
                    ApplicationArea = All;
                }
                field("UID No."; Rec."UID No.")
                {
                    ApplicationArea = all;
                }
    ...
    begin
        ...
        if FileName <> '' then
            ...
            ContactFieldRef := ContactRecordRef.Field(1);
            ContactFieldRef.Value := Rec."No.";
            if ContactRecordRef.Find('=') then;
            DocumentAttachment.SaveAttachmentFromStream(TempBlob.CreateInStream(), ContactRecordRef, FileName);
        end;
        CurrPage.Update(true);
    end;
