page 50000 "Amortization Entries"
{
    //Win593++

    AutoSplitKey = false;
    Caption = 'Payment Scheduled';
    DelayedInsert = false;
    PageType = List;
    UsageCategory = Lists;
    ApplicationArea = All;
    SaveValues = true;
    SourceTable = "Amortization Entry";
    ...
    layout
    {
        area(content)
        {
            repeater(General)
            {
                ...
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Begining Balance field.';
                }
                field("Prinipal Amount"; Rec."Prinipal Amount")
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Prinipal Amount field.';
                }
                ...
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Ending Balance field.';
                }
        ...
        area(navigation)
        {
            ...
            group("F&unction")
            {
                ...
                action("Apply &Entries")
                {
                    ...
                    trigger OnAction()
                    begin
                        AmortizationMgt.ApplyEntries(Rec);
                    end;
                }
            }
        }
        area(processing)
        {
                ...
                action(CreateCashJournal)
                {
                    ...
                    begin
                        ...
                        IF CONFIRM(Text001, FALSE) THEN BEGIN
                            //AmortizationMgt.Post(Rec);
                            CurrPage.SETSELECTIONFILTER(Rec);
                            PDC.COPY(Rec);
                            AmortizationMgt.CreateCashJnlandPostEntries(Rec, false);
                            //AmortizationMgt.PostJournal(Rec, FALSE);
                            /*CustomerNo := '';
                            DateFilter := '';
                            ContractFilter := '';
                ...
                action("CreateCashJournal & Post")
                {
                    ...
                    begin
                        ...
                        IF CONFIRM(Text003, FALSE) THEN BEGIN
                            CurrPage.SETSELECTIONFILTER(Rec);
                            PDC.COPY(Rec);
                            //AmortizationMgt.PostJournal(Rec, TRUE);
                            AmortizationMgt.CreateCashJnlandPostEntries(Rec, true);
                        END;
            ...
            action("PDC Reciepts")
            {
                ...
                var
                    ...
                    RecPDCLine: Record "Amortization Entry";
                    LineNo: Integer;
                begin
                    //RecGenJournal.Reset();
                    ...
                    if RecPDCLine.FindSet() then
                        repeat
                            ...
                            RecGenJournal.Validate("Account No.", RecPDCLine."Account No.");
                            RecGenJournal.validate("Debit Amount", Round(RecPDCLine.Amount * -1));
                            RecGenJournal."Bal. Account Type" := RecGenJournal."Bal. Account Type"::Customer;
                            RecGenJournal.Validate("Bal. Account No.", RecPDCLine."Customer No.");
    ...
    begin
        UpdateBalance;
        //QuoteVisible := Rec."Document Type" = Rec."Document Type"::Order;
        QuoteVisible := false;
    end;
    ...
    var
        CustomerNo: Code[20];
        Customer: Record 18;
        PostDatedCheck: Record "Amortization Entry";
        GLAccount: Record 15;
        CustomerList: Page "Customer List";
        AmortizationMgt: Codeunit AmortizationMgt;
        ...
        PostDatedCheck1: Record "Amortization Entry";
