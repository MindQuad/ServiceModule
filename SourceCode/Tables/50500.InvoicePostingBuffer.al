table 50500 "Invoice Post. Buffer New"
{
    Caption = 'Invoice Post. Buffer New';
    ReplicateData = false;
    TableType = Normal;
    ...
    var
        TempInvoicePostBufferRounding: Record "Invoice Post. Buffer New" temporary;
    ...
    procedure FillPrepmtAdjBuffer(var TempInvoicePostBuffer: Record "Invoice Post. Buffer New" temporary; InvoicePostBuffer: Record "Invoice Post. Buffer New"; GLAccountNo: Code[20]; AdjAmount: Decimal; RoundingEntry: Boolean)
    begin
        TempInvoicePostBuffer.PreparePrepmtAdjBuffer(InvoicePostBuffer, GLAccountNo, AdjAmount, RoundingEntry);
    end;
#endif

    procedure PreparePrepmtAdjBuffer(InvoicePostBuffer: Record "Invoice Post. Buffer New"; GLAccountNo: Code[20]; AdjAmount: Decimal; RoundingEntry: Boolean)
    var
        PrepmtAdjInvoicePostBuffer: Record "Invoice Post. Buffer New";
    begin
    ...
    procedure Update(InvoicePostBuffer: Record "Invoice Post. Buffer New")
    var
    ...
    procedure Update(InvoicePostBuffer: Record "Invoice Post. Buffer New"; var InvDefLineNo: Integer; var DeferralLineNo: Integer)
    begin
    ...
    local procedure OnAfterInvPostBufferPrepareSales(var SalesLine: Record "Sales Line"; var InvoicePostBuffer: Record "Invoice Post. Buffer New")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInvPostBufferPreparePurchase(var PurchaseLine: Record "Purchase Line"; var InvoicePostBuffer: Record "Invoice Post. Buffer New")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInvPostBufferPrepareService(var ServiceLine: Record "Service Line"; var InvoicePostBuffer: Record "Invoice Post. Buffer New")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInvPostBufferModify(var InvoicePostBuffer: Record "Invoice Post. Buffer New"; FromInvoicePostBuffer: Record "Invoice Post. Buffer New")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInvPostBufferUpdate(var InvoicePostBuffer: Record "Invoice Post. Buffer New"; var FromInvoicePostBuffer: Record "Invoice Post. Buffer New")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetAmountsNoVAT(var InvoicePostBuffer: Record "Invoice Post. Buffer New"; TotalAmount: Decimal; TotalAmountACY: Decimal; VATDifference: Decimal)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalcDiscount(var InvoicePostBuffer: Record "Invoice Post. Buffer New"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalcDiscountNoVAT(var InvoicePostBuffer: Record "Invoice Post. Buffer New"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeInvPostBufferUpdate(var InvoicePostBuffer: Record "Invoice Post. Buffer New"; var FromInvoicePostBuffer: Record "Invoice Post. Buffer New")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeInvPostBufferModify(var InvoicePostBuffer: Record "Invoice Post. Buffer New"; FromInvoicePostBuffer: Record "Invoice Post. Buffer New")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforePrepareSales(var InvoicePostBuffer: Record "Invoice Post. Buffer New"; var SalesLine: Record "Sales Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFillPrepmtAdjBufferOnBeforeAssignInvoicePostBuffer(var PrepmtAdjInvPostBuffer: Record "Invoice Post. Buffer New"; InvoicePostBuffer: Record "Invoice Post. Buffer New")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCopyToGenJnlLine(var GenJnlLine: Record "Gen. Journal Line"; InvoicePostBuffer: Record "Invoice Post. Buffer New");
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCopyToGenJnlLineFA(var GenJnlLine: Record "Gen. Journal Line"; InvoicePostBuffer: Record "Invoice Post. Buffer New");
    begin
    end;
}
