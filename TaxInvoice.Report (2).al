report 50067 "Tax Invoice"
{
    DefaultLayout = RDLC;
    //RDLCLayout = './TaxInvoice.rdl';
    RDLCLayout = 'SourceCode/Report/TaxInvoice.rdl';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Service Invoice Header")
        {
            ...
            column(CustomerNo_PurchaseHeader; "Purchase Header"."Customer No.")
            {
            }
            column(PhoneNo_PurchaseHeader; "Purchase Header"."Ship-to Phone")
            {
            }
            column(EMail_PurchaseHeader; "Purchase Header"."Ship-to E-Mail")
            {
            }
            column(PostCode_PurchaseHeader; "Purchase Header"."Post Code")
            {
            }
            ...
            column(ShiptoCountry_PurchaseHeader; "Purchase Header"."Ship-to County")
            {
            }
            ...
            dataitem("Purch. Comment Line"; "Service Comment Line")
            {
                DataItemLink = "No." = FIELD("No.");
                DataItemTableView = SORTING("Table Name", "Table Subtype", "No.", Type, "Table Line No.", "Line No.")
                                    ORDER(Ascending)
                                    WHERE("Table Line No." = FILTER(= 0));
                column(DocumentType_PurchCommentLine; '')
                {
                }
                ...
                column(Code_PurchCommentLine; '')
                {
                }
                column(Comment_PurchCommentLine; "Purch. Comment Line".Comment)
                {
                }
                column(DocumentLineNo_PurchCommentLine; '')
                {
                }
            }
            dataitem("Purchase Line"; "Service Invoice Line")
            {
                ...
                column(DirectUnitCost_PurchaseLine; '')
                {
                }
                ...
                column(UnitCost_PurchaseLine; '')
                {
                }
                column(ExpectedReceiptDate_PurchaseLine; '')
                {
                }
            ...
            dataitem("Service Contract Header"; "Service Contract Header")
            {
                ...
                column(ContactNo_ServiceContractHeader; "Contact No.")
                {
                }
